...
namespace Business.Repositories.UserRepository
{
    public class UserManager : IUserService
    {
        ...
        public async Task<IDataResult<User>> Add(UserDto userDto)
        {
            try
            {
                ...
                {
                    fileName = _fileService.FileSaveToFtp(userDto.Image, "images");
                }
                ...
                var user = CreateUser(userDto, fileName);

                user.ConfirmValue = confirmValue;

                return new SuccessDataResult<User>(await _userDal.AddUser(user), UserMessages.Added);
            }
            catch
            {
                return new ErrorDataResult<User>(UserMessages.NotAdded);
            }
        ...
        private static User CreateUser(RegisterAuthDto registerDto, string fileName)
        {
            ...
            user.TypeId = registerDto.TypeId;
            user.Title = registerDto.Title;
            user.Task = registerDto.Task;
        ...
        public async Task<IResult> Update(UserDto userDto)
        {
            try
            {
                ...
                var user = await _userDal.Get(p => p.Id == userDto.Id);

                user.FirstName = userDto.FirstName;
                user.LastName = userDto.LastName;
                user.PhoneNumber = userDto.PhoneNumber;
                user.Title = userDto.Title;
                user.Task = userDto.Task;
            ...
            {
                return new ErrorResult(UserMessages.NotUpdated);
            }
        }
        ...
        public async Task<IResult> Delete(User user)
        {
            ...
            {
                return new ErrorResult(UserMessages.NotDeleted);
            }
        }
        ...
        {
            return new SuccessDataResult<List<User>>(await _userDal.GetAll(x => x.TypeId == 1));
        }
        ...
        {
            var user = await _userDal.Get(p => p.Id == userChangePasswordDto.Id);
        ...
        public async Task<IResult> UpdatePersonalInformation(UserDto userDto)
        {
            try
            {
                User user = await _userDal.Get(p => p.Id == userDto.Id);
        ...
        public async Task<IResult> UpdatePassword(UserChangePasswordDto userDto)
        {
            try
            {
                var user = await _userDal.Get(p => p.Id == userDto.Id);
                bool result = HashingHelper.VerifyPasswordHash(userDto.OldPassword, user.PasswordHash, user.PasswordSalt);
                if (!result)
                {
                    return new ErrorResult(UserMessages.WrongCurrentPassword);
                }
                ...
                byte[] passwordHash, paswordSalt;
                HashingHelper.CreatePassword(userDto.NewPassword, out passwordHash, out paswordSalt);
            ...
            {
                return new ErrorResult(UserMessages.NotChangePassword);
            }
        }

        public async Task<IResult> UpdateProfilePhoto(UserPhotoDto userDto)
        {
            try
            {
                string fileName = _fileService.FileSaveToFtp(userDto.Image, "images");
                User user = await _userDal.Get(p => p.Id == userDto.Id);
                user.ImageUrl = fileName;
